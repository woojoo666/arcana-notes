// preprocessing
function preprocessor(filetext) {
...
function indentationProcessing(filetext) {
...
function tokenizer (block, indentChar) {
	let lines = block.split('\n').filter(str => ! /^\s*$/.test(str));   // split into lines and filter out empty lines
...
// detect if a file has mixed spaces and tabs
function detectMixedSpacesAndTabs(filetext, indentChar) {
	let leadingwhitespace = filetext.match(/(^|\n)\s*/g);
	let whitespaceWithoutIndents = leadingwhitespace.split(indentChar).join('');
	return whitespaceWithoutIndents.length > 0;
}
...
//    * when indentation decreases, insert a DEDfdsaENT token "}" and a comma ","
