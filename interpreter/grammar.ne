...
Statement	-> %identifier ":" Expression							{% ([token, ,value]) => ({type:'property', key: token.value, value}) %} # (TODO: support destructuring)
			...
			| Object												{% (items) => ({type:'subList', items}) %} # TODO: hack for now, since list items and cloning are ambiguous (from parser's point of view, "x(y)" can be two list items or one clone)
...
Object 		-> "(" Block ")"										{% ([ ,block, ]) => ({type:'create',block}) %}
			...
			| %selfRef												{% ([token]) => ({type:'reference', name: token.value, selfRef: true}) %}
